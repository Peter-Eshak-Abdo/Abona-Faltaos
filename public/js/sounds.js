const sounds = [
  ...
  },
  {
    ...
    duration: "7:06",
    src: "../media/sound/al7an/الهوس الاول قبطي.mp3",
  },
  {
    monasba: "keahk",
  ...
  // let la7nSomKebir0 = document.getElementById("la7n-som-kebir0");
    ...
    async function loadSound() {
      try {
        ...
        const la7nSomKebir = new Audio(URL.createObjectURL(await response.blob()));

        la7nSomKebir.addEventListener("loadedmetadata", () => {
          la7nSomKebir.forEach((info) => {
            let la7nSomKebirDur = document.querySelectorAll("#la7n-som-kebir-duration");
            la7nSomKebirDur.innerHTML = (info.duration) / 60;
          });
